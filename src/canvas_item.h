 ...
 * такого объекта является "резинка" и инструмент измерения расстояния.
 *
...
protected:
     ...
     * Удаляет старый вид объекта и рисует новый, с помощью вызова xdraw. Методу передается
     * будущий размер объекта, результат функции xdraw не должен выходить за границы
     * переданного параметра.
     */
    void new_geometry(const QRect& r);
private:
    QRect currentRect;
};
